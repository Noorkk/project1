...
from wtforms import StringField, IntegerField, TextAreaField, HiddenField,PasswordField,BooleanField
...
def load_user(user_id):
    return Checkout.query.get(int(user_id))
...
class RegisterForm(FlaskForm):
    ...
    username = StringField('username', validators=[InputRequired(), Length(min=4, max=15)])
    password = PasswordField('password', validators=[InputRequired(), Length(min=8, max=80)])
    phone_number=PasswordField('phone number', validators=[InputRequired(), Length(min=8, max=80)])
...
def login():
    ...
    if form.validate_on_submit():
        user = Checkout.query.filter_by(email=form.email.data).first()
        if user:
...
def signup():
    ...
    if form.validate_on_submit():
        ...
        new_user = Checkout(username=form.username.data, email=form.email.data, address=form.address.data, password=hashed_password)
        db.session.add(new_user)
        db.session.commit()

        flash("Registration was sucessfull , Please Login")
